...
- [x] 자체 인덱싱 (MiniSearch + 한국어 n-gram, postbuild) — Pagefind 거절: dynamic 라우트 비호환
- [x] 검색 UI (Cmd+K 단축키, shadcn Command 컴포넌트)
- [x] 검색 결과 → 노드 페이지 이동
- [ ] 코드 블록 복사 버튼
- [ ] 헤더 앵커 + 목차 (TOC, 우측)
- [x] 다음/이전 노드 네비
...
**진행 상황**: A 묶음(검색) 완료 — 2026-05-09. 다음은 B(노드 페이지 마감 — TOC·코드 복사·다크모드).
